...
- **`Dumbbell.reconnect()`.** Tears down the old `_ble`, constructs a fresh transport via `_transportFactory(device)` (defaults to `BleConnection.new`), re-wires the lazy `connectionState` forwarder if listeners are attached, then runs the normal `connect()` flow. `_ble` is no longer `final`; subscribers to `Dumbbell.connectionState` / `Dumbbell.states` stay attached across the swap via Dumbbell-owned controllers. `Dumbbell.handleTransportDrop()` and `Dumbbell.reconnect()` are public methods documented as WeightGroup-internal — only the supervisor should call them.
- **`connectionState` late-subscribe replay.** The proxy controller is a broadcast, so it doesn't replay past events to late subscribers — but the underlying `_ble.connectionState` does (the plugin is a BehaviorSubject). `Dumbbell` caches every forwarded value in `_latestConn` and the public `connectionState` getter returns a `Stream.multi` that replays the cached state to each new listener before forwarding live updates. Without this cache a `DumbbellCard` that builds *after* `WeightGroup` has already wired its supervisor listener would sit on `null` until the next transport transition — rendering "Connecting…" for an already-connected dumbbell.
...
- **`Dumbbell.refresh()`.** Sends a `0xD1 queryStatus` frame on TX; the firmware reply lands on the existing RX subscription and re-emits through `Dumbbell.states` like any other state frame. No-op on a disposed or not-yet-ready dumbbell — `isReady` gating mirrors `setWeightIndex` so a refresh during the connect window can't `StateError` the underlying transport.
- **`WeightGroup.refresh()`.** Fans out to every ready member in parallel via `Future.wait(eagerError: false)`. Not-yet-ready and mid-reconnect members are skipped (the same defence-in-depth filter as `setWeightIndex`). One flaky peer doesn't suppress the others' refresh. No-op on a disposed group or one with no ready members.
- **`RefreshIndicator` on the top region.** Pulling down on the user's selected-dumbbells region fires `HomeScreen._onRefresh`, which kicks `WeightGroup.refresh()` and the existing `_startScan()` (stop-then-start) in parallel. The Material spinner stays until both complete; fresh state frames and new scan results continue to arrive through their normal streams after that.
...
- ✅ **Permission rationale's first-launch behaviour on iOS.** ✅ Resolved (verified on iOS 16.7.11). The fix was two-part: the previous code requested `Permission.bluetoothScan` + `Permission.bluetoothConnect`, which on iOS route through `UnknownPermissionStrategy` in `permission_handler_apple` and always report `permanentlyDenied`. Switching to `Permission.bluetooth` on iOS (via `lib/state/bluetooth_permissions.dart`'s `defaultTargetPlatform` branch) is necessary but not sufficient: `permission_handler_apple`'s real `BluetoothPermissionStrategy` is gated behind a `PERMISSION_BLUETOOTH=1` preprocessor macro, which the Flutter-generated Podfile does not set by default. Without it, `Permission.bluetooth` *also* falls through to `UnknownPermissionStrategy` and always reports denied. `ios/Podfile`'s `post_install` hook now adds `PERMISSION_BLUETOOTH=1` to `GCC_PREPROCESSOR_DEFINITIONS` for every Pod target. Per-checkout, this requires `(cd ios && pod install)` after the macro lands. Android keeps the runtime split (`bluetoothScan` + `bluetoothConnect`); the legacy `Permission.bluetooth` is the pre-Android-12 manifest permission and wouldn't trigger the right prompt there.
