...
Shipped so far: §2a (state-stream robustness — auto-reconnect on drop + resume kick), §2b (UX polish — animations + haptic + reconnect-failure SnackBar + activity glyph), §2c (About screen restructure), §2d (Design v1), §2g (pull-down to refresh — `Dumbbell.refresh` + `WeightGroup.refresh` + top-region `RefreshIndicator`). Outstanding: §2e (rename dumbbells), §2f (per-device weight override).
...
3. 🟡 Phase 2 — UX and UI improvements. §2a (state-stream robustness — auto-reconnect on drop + resume kick) + §2b (UX polish — animations + haptic + reconnect SnackBar) + §2c (About screen) + §2d (Design v1) + §2g (pull-down to refresh) shipped. Still pending: §2e (rename dumbbells), §2f (per-device weight override).
