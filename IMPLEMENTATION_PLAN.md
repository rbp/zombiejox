...
Total test count: **245 tests, all passing.** `flutter analyze` clean. `dart format` clean. All tests above `lib/ble/` use the port types — no test imports `package:flutter_blue_plus/` outside the adapter.
...
- **Accessibility.** The message is wrapped in `Semantics(container: true, liveRegion: true, ...)` so screen readers announce the toast on appearance — without it, AT users tap the chip and get no audible response before the toast auto-dismisses. Both card chips also get a 48 dp Material-minimum touch target (`SizedBox(height: 48) → Center → _StatusChip`) so the tap region matches the hit-target guideline while the chip's visual size stays unchanged.
