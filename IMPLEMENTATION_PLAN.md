...
Total test count: **244 tests, all passing.** `flutter analyze` clean. `dart format` clean. All tests above `lib/ble/` use the port types — no test imports `package:flutter_blue_plus/` outside the adapter.
...
- **Toast widget, not SnackBar.** `lib/widgets/status_toast.dart` exposes `showStatusToast(BuildContext, String)`, which inserts an `OverlayEntry` containing a centered `Material` pill that fades in over 200 ms, holds for 1.6 s, then fades out over 200 ms and removes itself. Wrapped in `IgnorePointer` so the toast doesn't block taps on the cards / weight grid beneath it — the user can tap the chip again, tap × on a card, or hit a weight tile while a toast is on screen. Centered rather than bottom-anchored (SnackBar) per the spec — feels closer to the old Android `Toast` UX, and doesn't push the layout around.
...
- **`SelectionModel.customNameFor(DeviceRef)`.** New accessor returning the user-set name (trimmed, null when absent or whitespace-only). Distinct from `displayNameFor`, which always returns a non-null fallback. Consults the in-selection entry first, then falls back to `Preferences.customDeviceNames` so a previously-renamed dumbbell that's been removed from the selection still annotates its toast on the next promote.
